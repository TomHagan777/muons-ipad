...
let muonInData = inData;

  if (muonInData == muonInData2){ //this syncs the count on detector with particle creation
      muon++;
      ...
      LEDoff = false 
      console.log(bigStar.length + '/' + muon); //handy: 'how many flared stars are created vs all stars in console log'
      
      muonInData2++;   //vital: muon2 is always one ahead of muon, kicking us out of this star creation conditional
    } 
    
if (muonInData == 255){ // sending values in bytes across serial has a max of 255… 
    muonInData = 0;     // …this resets the arduino count without diminishing the overall count
    muonInData2 = 1;
}  

if (muonInData > muonInData2){  // sometimes the muons are passed too fast for this script to detect… 
    muonInData2 = muonInData; // …this ensures that if muonInData is more than our ++ control…
    console.log("Double Muon!") // …the counter control is iterated back upward to meet the muonInData.
}
